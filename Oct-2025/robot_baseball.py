"""
Jane Street Puzzle (Oct 2025) - Robot Baseball
----------------------------------------------

Goal: solve for optimal pitcher/batter strategies in a simplified robot baseball
game, then compute the maximum probability of reaching a full count.

What the script does (at a glance):
- Symbolic stage: build payoff/probability expressions with Sympy, derive Nash
  equilibrium strategies (a0 for batter wait-prob, b0 for pitcher ball-prob),
  and the resulting expected score x0.
- Dynamic programming: fill tables of values/strategies for each count state
  (balls i, strikes j) by substituting future-state expressions into x0, a0,
  b0, and q.
- Optimization: numerically maximize q at the initial state over pitch
  accuracy p using SciPy’s `minimize`, capturing the best p0 and q0.
- Plotting: graph q as a function of p and mark the optimum.

Files: this script is standalone; outputs are printed and a matplotlib plot is
shown for the q(p) curve.
"""
